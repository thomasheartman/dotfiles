...
**** TODO macOS                                                       :macos:
     ...
     :END:
     TODO: rewrite this to use noweb references so that you don't need
     all the duplicate ~when~ statements. In that case, though, you
     might also need to disable tangling for for the extra options.
...
* TODO [#C] Window movement and management
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    _↓_: ↓    ___: below  _d_: ace-delete
    _↑_: ↑    _u_: undo   _k_: del window
    _→_: →    _r_: redo
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package exwm
      ...
      (defun my-exwm-launcher (command)
        "A copy of the spacemacs exwm launcher: https://github.com/djeis97/spacemacs-configs/blob/a10ddd940bca28f9d8be7fa900761344ea611a5f/layers/exwm/packages.el#L172"
      ...
      (setq exwm-input-global-keys
        `(([?\s-r] . exwm-reset)
          ([?\s- ] . my-exwm-launcher)
           ...
           ([?\C-\M-\S-\s-c] . windmove-up)
           ([?\C-\M-\S-\s-t] . windmove-down)
           ([?\C-\M-\S-\s-n] . windmove-right)
           ([?\C-\M-\S-\s-h] . windmove-left)
